...
function get_simple_likes_button( $post_id, $is_comment = NULL ) {
	...
	if ( already_liked( $post_id, $is_comment ) ) {
		...
		$title = __( 'data-toggle="tooltip" data-placement="bottom" title="" data-container="body" data-original-title="取消喜欢"', 'loobo' );
		$icon = $icon_full;
	} else {
		$class = '';
		$title = __( 'data-toggle="tooltip" data-placement="bottom" title="" data-container="body" data-original-title="喜欢"', 'loobo' );
		$icon = $icon_empty;
	}
	$output = '<a href="' . admin_url( 'admin-ajax.php?action=process_simple_like' . '&post_id=' . $post_id . '&nonce=' . $nonce . '&is_comment=' . $is_comment . '&disabled=true' ) . '" class="btn btn-white btn-simple sl-button' . $post_id_class . $class . $comment_class . '" data-nonce="' . $nonce . '" data-post-id="' . $post_id . '" data-iscomment="' . $is_comment . '" ' . $title . '>  ' . $count . '</a>' . $loader . '<div class="ripple-container"></div>';
	return $output;
} 
...
function get_liked_icon() {
	$icon = '<span class="sl-icon"><i class="iconfont icon-like1-o"></i></span>';
	return $icon;
} 

function get_unliked_icon() {
	$icon = '<span class="sl-icon"><i class="iconfont icon-like1"></i></span>';
	return $icon;
}
function get_comment_icon() {
	$icon = '<span class="sl-icon"><i class="iconfont icon-pinglun"></i></span>';
	return $icon;
} 
function get_collect_icon() {
	$icon = '<span class="sl-icon"><i class="iconfont icon-xingxing-copy"></i></span>';
	return $icon;
} 
...
function get_like_count( $like_count ) {
	$like_text = __( 'Like', 'loobo' );
	$number = sl_format_count( $like_count );
	$count = '<i class="material-icons">thumb_up</i> <span class="sl-count">' . $number . '</span>';
	return $count;
} 
function show_user_likes( $user ) { ?>        
	        ...
	        <div id="post-<?php the_ID(); ?>" class="col-md-4">
			    <div class="card card-background" style="background-image: url(<?php echo get_post_thumbnail();?>)">
			...
			<?php $sep = ''; endwhile;  else :  endif;  wp_reset_postdata();  ?>
<?php } 