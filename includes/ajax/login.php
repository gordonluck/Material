...
if($_POST['action']){ 
	...
	if($action == 'lb_login'){
		...
		if ( $sanitized_user_login == '' ) {
		  ...
		  if(empty($_POST['captcha']) || empty($_SESSION['MBT_mohtml_captcha']) || trim(strtolower($_POST['captcha'])) != $_SESSION['MBT_mohtml_captcha']){
		  	  $error = '验证码错误 ';
		  }
		  if($_SESSION['MBT_mohtml_captcha_email'] != $user_email){
		  	  $error = '验证码与邮箱不对应';
		  }
		  ...
		  if($error){ echo $error;}
		  	...
		  	unset($_SESSION['MBT_mohtml_captcha']);
		  	unset($_SESSION['MBT_mohtml_captcha_email']);
