...
if($_POST['action'] == 'lb_captcha' && filter_var($_POST['email'], FILTER_VALIDATE_EMAIL)){
...
function sessioncode($email){
	...
	$_SESSION['MBT_mohtml_captcha']=strtolower($_dscode);
	$_SESSION['MBT_mohtml_captcha_email']=$email;
	$message .= '验证码：'.$_dscode;   
	wp_mail($email, '注册验证码-'.get_bloginfo('name'), $message);    
}
